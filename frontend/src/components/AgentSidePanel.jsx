    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Slide-out Left Side Panel (~26% width) */}
          <motion.div
            initial={{ x: '-100%' }}
            animate={{ x: 0 }}
            exit={{ x: '-100%' }}
            ...
            style={{
              ...
              width: '26vw', minWidth: 320, maxWidth: 410,
          ...
          >
            ...
            <div style={{ padding: '16px 24px', background: 'rgba(17, 24, 39, 0.65)', borderBottom: '1px solid var(--border-muted)' }}>
              <button
            ...
            <div style={{ padding: '16px 24px 8px 24px', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {['All', 'Powertrain', 'Safety', 'Hardware'].map(cat => (
                <button
                  key={cat}
                  onClick={() => setActiveFilter(cat)}
                  style={{
                    padding: '6px 12px', fontSize: 11, fontWeight: 700, borderRadius: 14,
            ...
            {/* Agent Cards Scroll List (16px vertical gaps) */}
