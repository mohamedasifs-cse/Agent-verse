...
export default function ReportsPanel({ telemetry, vehicleName = 'TATA Safari EV', agentResults }) {
  ...
  const totalSessions = analyticsData?.charging_history_summary?.total_sessions ?? 24;
  const avgCostPerSession = analyticsData?.charging_history_summary?.avg_cost_per_session
    ? Math.round(analyticsData.charging_history_summary.avg_cost_per_session > 100 ? analyticsData.charging_history_summary.avg_cost_per_session : analyticsData.charging_history_summary.avg_cost_per_session * 83)
    : 420;
  const totalEnergyKwh = analyticsData?.charging_history_summary?.total_energy_kwh ?? (telemetry?.totalEnergyChargedKwh || 342.5);
  const preferredTime = analyticsData?.charging_history_summary?.preferred_charging_time ?? 'Off-Peak (11:00 PM - 06:00 AM)';
  ...
  // Cost metrics (INR ₹)
  const estTripCost = pricingData?.estimated_trip_cost ? Math.round(pricingData.estimated_trip_cost) : Math.round(dist * 0.16 * 16);
  const petrolCost = Math.round((dist / 14) * 102);
  const savingsVsPetrol = Math.max(0, petrolCost - estTripCost);
  const costPerKm = (estTripCost / Math.max(1, dist)).toFixed(2);
  const cheapestStation = pricingData?.cheapest_station?.name || 'Tata Power EZ Charge - Highway Hub';
...
[ 5. ECONOMIC SAVINGS (INR ₹) ]
----------------------------------------------------------------
Estimated Trip Cost : ₹${estTripCost}
Savings vs Petrol   : ₹${savingsVsPetrol}
Operating Cost / km : ₹${costPerKm} / km
Best Value Charger  : ${cheapestStation}

[ 6. PREDICTIVE HARDWARE MAINTENANCE ]
----------------------------------------------------------------
...
`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 960, margin: '0 auto', color: 'var(--text-primary)' }}>
      ...
      {/* ── Cost Analysis (INR ₹) ── */}
      <div className="rydex-card" style={{ padding: 20 }}>
        <div className="section-label" style={{ marginBottom: 14 }}>4. Economic & Financial Cost Analysis (INR ₹)</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 16 }}>
          {[
            { label: 'Est. Trip Charging Cost', value: `₹${estTripCost}`, accent: '#00f0ff' },
            { label: 'Net Savings vs Petrol', value: `₹${savingsVsPetrol}`, accent: 'var(--success)' },
            { label: 'Operating Cost per km', value: `₹${costPerKm} / km`, accent: '#fff' },
            { label: 'Cheapest Station Hub', value: cheapestStation, accent: '#d4d414' },
          ].map(({ label, value, accent }) => (
            <div key={label} style={{ background: 'rgba(0,0,0,0.3)', padding: 14, borderRadius: 10, border: '1px solid var(--border-muted)' }}>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>{label}</div>
              <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 17, color: accent }}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rydex-card" style={{ padding: 20, border: '1px solid rgba(34,197,94,0.35)' }}>
        <div className="section-label" style={{ marginBottom: 14, color: 'var(--success)' }}>
          <span style={{ '--section-accent': 'var(--success)' }}>5. Environmental & Sustainability Impact</span>
        </div>
      ...
      <div className="rydex-card" style={{ padding: 20 }}>
        <div className="section-label" style={{ marginBottom: 14 }}>6. Predictive Hardware Maintenance Insights</div>
