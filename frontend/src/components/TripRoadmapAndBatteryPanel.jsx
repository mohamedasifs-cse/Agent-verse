      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
        ...
        <div style={{ background: 'rgba(0, 240, 255, 0.06)', padding: 14, borderRadius: 10, border: '1px solid rgba(0, 240, 255, 0.35)' }}>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>
            OVERALL TRIP CHARGE NEEDED
          </div>
          {/* OVERALL PERCENTAGE IS BIG */}
          <div style={{ fontFamily: 'var(--font-heading)', fontSize: 26, fontWeight: 900, color: '#00f0ff', lineHeight: 1.1 }}>
            {overallTripSocNeeded}% <span style={{ fontSize: 16, fontWeight: 700 }}>SoC</span>
          </div>
          {/* kWh IS SMALL BELOW */}
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4, fontWeight: 600 }}>
            ⚡ {initialEnergyNeededKwh} kWh total energy required
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.4)', padding: 14, borderRadius: 10, border: '1px solid var(--border-muted)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-heading)', fontSize: 26, fontWeight: 900, color: 'var(--success)', lineHeight: 1.1 }}>
            {arrivalSocWithStopPct}% <span style={{ fontSize: 16, fontWeight: 700 }}>SoC</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4, fontWeight: 600 }}>
            {isChargingNeeded ? `With Charge Stop (Overall Need: ${overallTripSocNeeded}% SoC)` : `Current: ${currentSoc}% · Direct Arrival: ${directArrivalSocPct}%`}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, position: 'relative', paddingLeft: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ background: 'rgba(15, 23, 42, 0.75)', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(34, 197, 94, 0.35)', flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>
                Predicted Reserve SoC: <strong style={{ color: 'var(--success)' }}>{arrivalSocWithStopPct}%</strong> {isChargingNeeded ? `(with 18-min stop · ${directArrivalSocPct}% direct)` : ''} · Remaining: <strong>{liveRemainingKm} km</strong>
              </div>
            </div>
          </div>
        </div>
