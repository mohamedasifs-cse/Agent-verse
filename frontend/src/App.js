...
function TelemetryCard({ label, value, unit, accentColor, icon, decimals = 1 }) {
  ...
  return (
    <div className="rydex-card hover-lift" style={{ padding: '16px' }}>
      <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 500, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 10 }}>
        {icon}&nbsp; {label}
      </div>
...
function DarkCard({ children, className = '', style = {}, accent = false }) {
  return (
    <div className={accent ? 'rydex-card-accent' : 'rydex-card'} style={{ padding: '20px', ...style }}>
      {children}
    </div>
  );
}
...
export default function App() {
  const {
    ...
    setSimulatorMode, runAnalysis, fetchStations,
  } = useEVSystem();
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', fontFamily: 'var(--font-body)', color: 'var(--text-primary)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255, 255, 255, 0.04)', padding: '6px 14px', borderRadius: 20, border: '1px solid var(--border-muted)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, fontWeight: 900, color: '#00f0ff' }}>{telemetry ? Math.round(telemetry.soc) : 80}%</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255, 255, 255, 0.04)', padding: '6px 14px', borderRadius: 20, border: '1px solid var(--border-muted)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, fontWeight: 900, color: '#fff' }}>{telemetry ? Math.round(telemetry.estimatedRangeKm) : 400} km</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255, 255, 255, 0.04)', padding: '6px 14px', borderRadius: 20, border: '1px solid var(--border-muted)' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, fontWeight: 900, color: '#d4d414' }}>{stations.length || 0} Hubs</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Main Content ───────────────────────────────────────────────────── */}
      <main style={{ padding: '28px 32px', maxWidth: '100%' }}>
            ...
            <motion.div key="map" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
              <DarkCard style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span className="rydex-badge accent">{stations.length} stations</span>
                    {origin && <span className="rydex-badge">Near {origin.display_name?.split(',')[0]}</span>}
                    {isDriving && <span className="rydex-badge success">🚗 Live Driving Simulation</span>}
                  </div>
                </div>
              ...
              {stations.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
                  {stations.map(s => (
                    <DarkCard key={s.id} className="hover-lift" style={{ padding: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                        <div style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 14 }}>{s.name}</div>
                        <span className={`rydex-badge ${s.available_bays > 0 ? 'success' : 'danger'}`}>
                          {s.available_bays}/{s.total_bays}
                        </span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, color: 'var(--text-secondary)' }}>
                        <div>📍 {s.distance_km} km away</div>
                        <div>⚡ <span style={{ color: 'var(--accent)' }}>{s.max_power_kw} kW</span> max</div>
                        <div>💰 <span style={{ color: 'var(--text-primary)' }}>₹{s.price_per_kwh}/kWh</span></div>
                        {s.queue_length > 0 && <div>🕐 Queue: {s.queue_length}</div>}
                        {s.is_green && <div style={{ color: 'var(--success)' }}>🌱 Green energy</div>}
                      </div>
                    </DarkCard>
                  ))}
                </div>
